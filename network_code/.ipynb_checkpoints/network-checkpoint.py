...
import numpy as np
from my_code.utils import Connectivity
...
def build_network(net_params, plastic_ee=False, heterogenous=False):
    ...
    all_eqs = neuron_eqs_nondep + neuron_eqs_common + curr_eqs
    if plastic_ee:
    ...
    '''
        ...
        rang=clip(1 + 0.25 * randn(),0,inf)
        syn_pp = Synapses(pop_p, pop_p, 'rang : 1', on_pre='g_p += g_ip*rang', name='syn_pp')
        syn_pp.connect(i=conn_pp.pre_index, j=conn_pp.post_index)
        syn_pp.rang=clip(1 + 0.25 * randn(),0,inf)
        syn_pp.delay = tau_l*clip(1 + 0.25 * randn(), 0, inf)
